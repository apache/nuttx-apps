...
static void snd_pcm_deinit(FAR snd_pcm_t *pcm)
{
  if (pcm->fd < 0)
    {
      return;
...
static int snd_pcm_init(FAR snd_pcm_t *pcm)
{
  char path[32];
  int ret;
...
static int snd_pcm_poll_available(FAR snd_pcm_t *pcm)
{
...
static int snd_pcm_peek_buffer(FAR snd_pcm_t *pcm,
                               FAR struct ap_buffer_s **buffer)
{
  int ret;
...
#define SCALAR(name, type)                                                \
  static void scalar_##name(FAR uint8_t *dst, FAR const uint8_t *src,     \
...
static void snd_pcm_voluem_scalar(FAR snd_pcm_t *pcm, FAR uint8_t *dest,
                                  FAR const uint8_t *src, int len)
{
  switch (pcm->format)
    {
    case SND_PCM_FORMAT_S16:
...
static int snd_pcm_enqueue_buffer(FAR snd_pcm_t *pcm,
                                  FAR struct ap_buffer_s *buffer)
{
                 ...
                 snd_pcm_stream_t stream, int mode)
{
  ...
  *pcm = (FAR snd_pcm_t *)malloc(sizeof(snd_pcm_t));
  if (!*pcm)
    {
      return -ENOMEM;
    }
...
int snd_pcm_pause(FAR snd_pcm_t *pcm, int enable)
{
  ...
  ret = snd_pcm_ioctl(pcm->fd, enable ? AUDIOIOC_PAUSE : AUDIOIOC_RESUME, 0);
...
{
  FAR union snd_interval *interval = NULL;
...
{
  FAR const union snd_interval *interval;
