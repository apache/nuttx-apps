/****************************************************************************
 * apps/examples/elf/main/mk_pkg_fixture.c
 *
...
static int write_index_file(const char *path, const char *arch,
                            const char *compat, const char *digest)
{
  const char *artifact = "hello";
  FILE *stream = fopen(path, "w");
  ...
  if (fprintf(stream,
              ...
              "\"packages\":["
              "{\"name\":\"hello\",\"version\":\"1.0.0\",\"arch\":\"%s\","
              "\"compat\":\"%s\",\"artifact\":\"%s\",\"sha256\":\"%s\","
              "\"type\":\"elf\"},"
              "{\"name\":\"hello\",\"version\":\"9.9.9\",\"arch\":\"arm\","
              "\"compat\":\"stm32f4discovery\",\"artifact\":\"%s\","
              "\"sha256\":\"%s\",\"type\":\"elf\"}"
              "]}\n",
              arch, compat, artifact, digest, artifact, digest) < 0)
    {
      fprintf(stderr, "failed to write %s: %s\n", path, strerror(errno));
      fclose(stream);
      return -1;
...
static int write_bad_index_file(const char *path, const char *arch,
                                const char *compat)
{
  FILE *stream = fopen(path, "w");
  ...
  if (fprintf(stream,
              "{\"_nxpkg_source\":\"/etc/nxpkg/index.json\","
              "\"packages\":["
              "{\"name\":\"hello-missing\",\"version\":\"1.0.0\","
              "\"arch\":\"%s\",\"compat\":\"%s\","
              "\"artifact\":\"hello-missing\","
              "\"sha256\":\""
...
{
  fprintf(stderr,
          "usage: %s <hello-bin> <index-json> <bad-index-json> "
...
int main(int argc, char *argv[])
{
  ...
  if (sha256_file(argv[1], digest, sizeof(digest)) < 0 ||
      write_index_file(argv[2], argv[6], argv[7], digest) < 0 ||
      write_bad_index_file(argv[3], argv[6], argv[7]) < 0 ||
      write_text_file(argv[4], g_pkgtest) < 0 ||
      write_text_file(argv[5], g_pkgfail) < 0)
    {
      return EXIT_FAILURE;
    }
