...
static int RegisterAppClass(HINSTANCE hInstance)
{
  WNDCLASS wc;
...
static HWND CreateAppWindow(void)
{
  HWND hwnd;
  static int nextid = 1;
  int width, height;
...
#  endif
    )
    {
      ...
      CreateWindowEx(0L, "EDIT",
                     "OK",
                     WS_BORDER | WS_CHILD | WS_VISIBLE,
                     width * 5 / 8, 10, 100, 18, hwnd, (HMENU) 5, NULL, NULL);
      ...
      HWND hlist = CreateWindowEx(0L, "LISTBOX",
                                  "OK",
                                  WS_HSCROLL | WS_VSCROLL | WS_BORDER | WS_CHILD
                                  | WS_VISIBLE,
                                  width * 5 / 8, 54, 100, 48,
        ...
        if (hdc != NULL)
          {
            ...
            GdArcAngle(hdc->psd, x + w / 2, y + h / 2, w / 2, h / 2,
                       pData->startdegrees * 64, pData->enddegrees * 64, MWPIE);
            GdSetForegroundColor(hdc->psd, RGB(0, 0, 0));
            GdArcAngle(hdc->psd, x + w / 2, y + h / 2, w / 2, h / 2,
                       pData->startdegrees * 64, pData->enddegrees * 64,
                       MWARCOUTLINE);
