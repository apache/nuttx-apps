/****************************************************************************
 * apps/examples/nanoxcalc/nxcalc.c
 *
 ...
 *
 * Inspired by the nxclock example (Greg Haerr).
 *
...
static GR_GC_ID gc_button;
static GR_GC_ID gc_button_press;
static GR_GC_ID gc_border;
                  ...
                  for (c = 0; c < COLS && !found; c++)
                    {
                      ...
                      if (mx >= rc->x && mx <= rc->x + rc->width &&
                        ...
                        {
                          GrFillRect(win, gc_button_press, rc->x, rc->y,
                            rc->width, rc->height);
                          found = 1;
                        }
                    }
...
static void redraw(void)
{
  ...
  GrGetGCTextSize(gc_text, display, strlen(display), GR_TFTOP, &tw,
                   &th, &tb);
      ...
      for (c = 0; c < COLS; c++)
        {
          ...
          char *lbl = (idx <
                       (int)(sizeof(btn_labels) /
                             sizeof(btn_labels[0]))? btn_labels[idx] : NULL);
          ...
          if (lbl)
            {
              GrGetGCTextSize(gc_text, lbl, strlen(lbl), GR_TFTOP, &tw, &th,
                &tb);
              GrText(win, gc_text, rc->x + rc->width / 2 - tw / 2,
                rc->y + rc->height / 2 - 6, lbl, strlen(lbl), GR_TFTOP);
            }
...
static void press_button(const char *label)
{
  ...
  if (strcmp(label, "+") == 0 || strcmp(label, "-") == 0 || strcmp(label,
    "*") == 0 || strcmp(label, "/") == 0)
    {
