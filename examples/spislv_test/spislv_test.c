...
static int hexstr_to_bytes(const char *hexstr,
                          unsigned char *bytes,
                          size_t max_bytes)
{
 ...
 * @param config Pointer to the program configuration structure to populate.
 * @return 0 on success, -1 on failure.
 */

static int parse_arguments(int argc, char *argv[], program_config_t *config)
{
  int opt;
  ...
  while ((opt = getopt(argc, argv, "x:t:le")) != -1)
    {
      switch (opt)
      {
        case 'x':
          ...
          if (config->num_bytes <= 0 || config->num_bytes > TX_BUFFER_SIZE)
          {
            fprintf(stderr, "Error: Invalid number of bytes
                              specified for write mode.\n");
            return -1;
          }
        ...
        case 't':
          ...
          if (config->timeout_seconds <= 0)
            {
              fprintf(stderr, "Error: Timeout must be
                                a positive integer.\n");
              return -1;
            }
        ...
        case 'l':
          if (config->mode != MODE_INVALID)
          {
            fprintf(stderr, "Error: Multiple operation
                              modes specified.\n");
            return -1;
          }
        ...
        default:
          ...
          fprintf(stderr, "  %s -l [-t <timeout_seconds>]\n", argv[0]);
          fprintf(stderr, "  %s -e [-t <timeout_seconds>]\n", argv[0]);
          printf("Examples:\n");
  ...
  if (config->mode == MODE_WRITE)
  {
    if (optind >= argc)
    {
      fprintf(stderr, "Error: Missing hexadecimal bytes to send.\n");
    ...
    {
      fprintf(stderr, "Error: Hex string length must be %d characters for %d bytes.\n",
    ...
    int converted = hexstr_to_bytes(hex_input, config->tx_buffer, TX_BUFFER_SIZE);
...
static int write_mode(program_config_t *config, int fd)
{
  ...
  /* Inform that data will be queued for sending, including data */

  char data_str[3 * TX_BUFFER_SIZE + 1]; /* 2 chars per byte + space + null terminator */
  char *ptr = data_str;
  for (int i = 0; i < config->num_bytes; i++)
    {
      int len = snprintf(ptr, sizeof(data_str) - (ptr - data_str), "%02X ", config->tx_buffer[i]);
  ...
  *ptr = '\0';
  ...
  if (bytes_written < 0)
    ...
    {
      fprintf(stderr, "Error: Incomplete write. Expected: %d, Written: %zd\n", config->num_bytes, bytes_written);
      return -1;
    }
...
{
  printf("Slave: Listen-only mode activated. Waiting for data from master.\n");
  return 0;
}

/**
 * @brief Executes the echo mode: continuously echoes received data back to the master.
 *
 * @param config Pointer to the program configuration structure.
 * @param fd File descriptor for the SPI device.
 * @return 0 on success, -1 on failure.
...
{
  printf("Slave: Echo mode activated. Will echo received data until timeout.\n");
  return 0;
}

/**
 * @brief Reads data from the SPI device with a specified timeout.
 *        Depending on the mode, it either exits after the first read or continues (echo mode).
 *
 * @param config Pointer to the program configuration structure.
 * @param fd File descriptor for the SPI device.
...
static int read_with_timeout(program_config_t *config, int fd)
{
  ...
  /* Continue reading based on the mode */
      ...
      if (select_ret == -1)
      ...
      else if (select_ret == 0)
        {
          if (config->mode == MODE_ECHO)
            {
              printf("Communication timeout after %d seconds. No more data received from master.\n", config->timeout_seconds);
            }

          else
            {
              printf("Communication timeout after %d seconds. No data received from master.\n", config->timeout_seconds);
            }
      ...
      else
        {
          ...
          if (bytes_read < 0)
            ...
            {
              printf("Data received from master (%zd bytes): ", bytes_read);
              ...
              if (config->mode == MODE_ECHO)
                {
                  bytes_written = write(fd, buffer_rx, bytes_read);
                  ...
                  if (bytes_written < 0)
                    ...
                    {
                      fprintf(stderr, "Error: Incomplete write during echo. Expected: %zd, Written: %zd\n", bytes_read, bytes_written);
                      return -1;
                    }
              ...
              /* In Write or Listen-Only Mode, exit after first read */
...
/**
 * @brief Main function orchestrating the SPI slave operations
 * based on user input.
 *
 * @param argc Argument count.
 * @param argv Argument vector.
