/****************************************************************************
 * apps/games/NXDoom/src/doom/p_maputl.c
 *
...
fixed_t p_intercept_vector(divline_t *v2, divline_t *v1);
...
static intercepts_overrun_t g_intercepts_overrun[] =
{
  {4, NULL, false},
...
static void p_make_divline(line_t *li, divline_t *dl)
{
  dl->x = li->v1->x;
  dl->y = li->v1->y;
  dl->dx = li->dx;
...
static void intercepts_mem_overrun(int location, int value)
{
  int i;
  int offset;
  int index;
  ...
  while (g_intercepts_overrun[i].len != 0)
    {
      if (offset + g_intercepts_overrun[i].len > location)
        {
          addr = g_intercepts_overrun[i].addr;
          ...
          if (addr != NULL)
            {
              if (g_intercepts_overrun[i].int16_array)
                {
      ...
      offset += g_intercepts_overrun[i].len;
      ++i;
    }
}
...
static void intercepts_overrun(int num_intercepts, intercept_t *intercept)
{
  int location;
  ...
  intercepts_mem_overrun(location, intercept->frac);
  intercepts_mem_overrun(location + 4, intercept->isaline);
  intercepts_mem_overrun(location + 8, (intptr_t)intercept->d.thing);
}

/* pit_add_line_intercepts.
...
static boolean pit_add_line_intercepts(line_t *ld)
{
  int s1;
  int s2;
  fixed_t frac;
...
static boolean pit_add_thing_intercepts(mobj_t *thing)
{
  fixed_t x1;
  fixed_t y1;
  fixed_t x2;
...
/* p_traverse_intercepts
 * Returns true if the traverser function returns true for all lines.
 */

static boolean p_traverse_intercepts(traverser_t func, fixed_t maxfrac)
{
  int count;
  fixed_t dist;
  intercept_t *scan;
...
int p_point_on_line_side(fixed_t x, fixed_t y, line_t *line)
{
  ...
  if (!line->dy)
    {
      if (y <= line->v1->y) return line->dx < 0;
...
int p_box_on_line_side(fixed_t *tmbox, line_t *ld)
{
  ...
  switch (ld->slopetype)
    ...
    case ST_POSITIVE:
      p1 = p_point_on_line_side(tmbox[BOXLEFT], tmbox[BOXTOP], ld);
      p2 = p_point_on_line_side(tmbox[BOXRIGHT], tmbox[BOXBOTTOM], ld);
      break;

    case ST_NEGATIVE:
      p1 = p_point_on_line_side(tmbox[BOXRIGHT], tmbox[BOXTOP], ld);
      p2 = p_point_on_line_side(tmbox[BOXLEFT], tmbox[BOXBOTTOM], ld);
      break;
    }
...
void p_unset_thing_position(mobj_t *thing)
{
  ...
  if (!(thing->flags & MF_NOSECTOR))
    {
      /* inert things don't need to be in blockmap? unlink from subsector */
  ...
  if (!(thing->flags & MF_NOBLOCKMAP))
    {
      /* inert things don't need to be in blockmap unlink from block map */
...
/* p_block_lines_iterator
 * The validcount flags are used to avoid checking lines
 * that are marked in multiple mapblocks,
 * so increment validcount before the first call
 * to p_block_lines_iterator, then make one or more calls
 * to it.
 */
                        ...
                        int flags, boolean (*trav)(intercept_t *))
{
  ...
  return p_traverse_intercepts(trav, FRACUNIT);
}
