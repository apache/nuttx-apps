/****************************************************************************
 * apps/games/NXDoom/src/doom/st_stuff.c
 *
  ...
  if (st_statusbaron)
    {
      ...
      if (sbarr) v_draw_patch(ST_ARMSBGX, 0, sbarr);

      if (netgame) v_draw_patch(ST_FX, 0, faceback);

      v_restore_buffer();

      v_copy_rect(ST_X, 0, st_backing_screen, ST_WIDTH, ST_HEIGHT, ST_X,
                  ST_Y);
    }
}

static int st_calc_paint_offset(void)
{
  int health;
  static int lastcalc;
...
static void st_update_face_widget(void)
{
  int i;
  angle_t badguyangle;
  angle_t diffang;
          ...
          if (doevilgrin)
            {
              ...
              st_faceindex = st_calc_paint_offset() + ST_EVILGRINOFFSET;
            }
        }
    }
          ...
          if (plyr->health - st_oldhealth > ST_MUCHPAIN)
            {
              ...
              st_faceindex = st_calc_paint_offset() + ST_OUCHOFFSET;
            }
          else
            {
              badguyangle =
                  r_point_to_angle2(plyr->mo->x, plyr->mo->y,
              ...
              st_facecount = ST_TURNCOUNT;
              st_faceindex = st_calc_paint_offset();
      ...
      if (plyr->damagecount)
        {
          if (plyr->health - st_oldhealth > ST_MUCHPAIN)
          ...
          else
            {
              priority = 6;
              st_facecount = ST_TURNCOUNT;
              st_faceindex = st_calc_paint_offset() + ST_RAMPAGEOFFSET;
            }
        }
    }
  ...
  if (!st_facecount)
    {
      st_faceindex = st_calc_paint_offset() + (st_randomnumber % 3);
...
static void st_update_widgets(void)
{
...
static void st_do_palette_stuff(void)
{
  int palette;
  byte *pal;
  int cnt;
...
static void st_draw_widgets(boolean refresh)
{
  int i;
...
static void st_load_unload_graphics(load_callback_t callback)
{
  int i;
  int j;
  int facenum;
...
static void st_load_callback(const char *lumpname, patch_t **variable)
{
...
static void st_unload_graphics(void)
{
          ...
          if (cht_check_cheat(&cheat_god, ev->data2))
            {
              ...
              if (plyr->cheats & CF_GODMODE)
                {
                  if (plyr->mo) plyr->mo->health = deh_god_mode_health;
