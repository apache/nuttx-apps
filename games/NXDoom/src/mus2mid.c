/****************************************************************************
 * apps/games/NXDoom/src/mus2mid.c
 *
...
begin_packed_struct struct musheader
{
  byte id[4];
...
typedef struct musheader musheader;
...
static boolean write_time(unsigned int time, MEMFILE *midioutput)
{
...
static boolean write_end_track(MEMFILE *midioutput)
{
  byte endtrack[] =
    {
      0xff, 0x2f, 0x00
...
static boolean write_press_key(byte channel, byte key, byte velocity,
                             MEMFILE *midioutput)
{
...
static boolean write_release_key(byte channel, byte key, MEMFILE *midioutput)
{
...
static boolean write_pitch_wheel(byte channel, short wheel,
                                 MEMFILE *midioutput)
{
...
static boolean write_change_patch(byte channel, byte patch,
                                  MEMFILE *midioutput)
{
...
static boolean write_change_controller_valued(byte channel, byte control,
...
static boolean write_change_controller_valueless(byte channel, byte control,
                                               MEMFILE *midioutput)
{
...
static int allocate_midi_channel(void)
{
  int result;
  int max;
  int i;
...
static int get_midi_channel(int mus_channel, MEMFILE *midioutput)
{
      ...
      if (channel_map[mus_channel] == -1)
        {
          ...
          write_change_controller_valueless(channel_map[mus_channel], 0x7b,
                                          midioutput);
        }
...
static boolean read_mus_header(MEMFILE *file, musheader *header)
{
  boolean result;
...
boolean mus2mid(MEMFILE *musinput, MEMFILE *midioutput)
{
  ...
  if (!read_mus_header(musinput, &musfileheader))
    {
      return true;
    }
      ...
      while (!hitscoreend)
        {
          ...
          switch (event)
            ...
            case mus_presskey:
              ...
              if (write_press_key(channel, key, channelvelocities[channel],
                                midioutput))
                {
                  return true;
                }
            ...
            case mus_pitchwheel:
              ...
              if (write_pitch_wheel(channel, (short)(key * 64), midioutput))
                {
                  return true;
                }
            ...
            case mus_systemevent:
              ...
              if (write_change_controller_valueless(
            ...
            case mus_changecontroller:
              ...
              if (controllernumber == 0)
                {
                  if (write_change_patch(channel, controllervalue,
                                         midioutput))
                    {
                      return true;
                    }
