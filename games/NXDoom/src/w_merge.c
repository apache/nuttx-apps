/****************************************************************************
 * apps/games/NXDoom/src/w_merge.c
 *
...
static int find_in_list(searchlist_t *list, const char *name)
{
  ...
  for (i = 0; i < list->numlumps; ++i)
    {
      if (!strncasecmp(list->lumps[i]->name, name, 8)) return i;
    }

  return -1;
}

static boolean setup_list(searchlist_t *list, searchlist_t *src_list,
  ...
  if (sprite_frames == NULL)
    {
      ...
      sprite_frames = z_malloc(
        sizeof(*sprite_frames) * sprite_frames_alloced,
        PU_STATIC, NULL
      );
    }
...
static boolean valid_sprite_lump_name(char *name)
{
...
static sprite_frame_t *find_sprite_frame(char *name, int frame)
{
  ...
  if (num_sprite_frames >= sprite_frames_alloced)
    {
      ...
      newframes = z_malloc(
        sprite_frames_alloced * 2 * sizeof(*sprite_frames),
        PU_STATIC, NULL
      );
...
static boolean sprite_lump_needed(lumpinfo_t *lump)
{
  sprite_frame_t *sprite;
  int angle_num;
  int i;

  if (!valid_sprite_lump_name(lump->name))
    {
      return true;
    }
  ...
  sprite = find_sprite_frame(lump->name, lump->name[4]);
  ...
  if (angle_num == 0)
    {
      ...
      for (i = 0; i < 8; ++i)
        {
          if (sprite->angle_lumps[i] == lump) return true;
        }
    }
  else
    {
      ...
      if (sprite->angle_lumps[angle_num - 1] == lump) return true;
    }
  ...
  if (lump->name[6] == '\0') return false;

  sprite = find_sprite_frame(lump->name, lump->name[6]);
...
static void add_sprite_lump(lumpinfo_t *lump)
{
  sprite_frame_t *sprite;
  int angle_num;
  int i;
        ...
        case SECTION_FLATS:
          ...
          if (!strncasecmp(lump->name, "F_END", 8))
          ...
          else
            {
              ...
              lumpindex = find_in_list(&pwad_flats, lump->name);
...
static void w_nwt_add_lumps(searchlist_t *list)
{
  int i;
  ...
  for (i = 0; i < list->numlumps; ++i)
    {
      ...
      index = find_in_list(&pwad, list->lumps[i]->name);
...
void w_merge_file(const char *filename)
{
  ...
  if (w_add_file(filename) == NULL) return;
...
void w_nwt_dash_merge(const char *filename)
{
  ...
  for (i = 0; i < iwad_sprites.numlumps; ++i)
    {
      if (find_in_list(&pwad, iwad_sprites.lumps[i]->name) >= 0)
        {
