...
/* Open a fresh /dev/crypto session descriptor.  NuttX file descriptors are
 * owned by the task group, so the fd is never cached across calls: each
 * operation opens it in the task that uses it and closes it when done.
 */

static int dropbear_cryptodev_open(void)
{
  int fd;
  int cfd;
  ...
  /* Keep the descriptor out of the NSH shells Dropbear forks and execs for
   * each SSH session.
   */
...
static int dropbear_chacha(int cfd, FAR const unsigned char *key,
                           unsigned int seq, uint64_t counter,
...
static int dropbear_poly1305(int cfd, FAR const unsigned char *key,
                                     ...
                                     FAR void *cipher_state)
{
  ...
  /* Validate that the crypto device is reachable at cipher setup. */

  cfd = dropbear_cryptodev_open();
  if (cfd < 0)
    {
      return CRYPT_ERROR;
    }
                                     ...
                                     int direction)
{
  ...
  if (dropbear_chacha(cfd, KEY_MAIN(state), seq, 0, zero, key,
                      sizeof(key)) != CRYPT_OK)
    {
      goto out;
    }
  ...
  if (direction == LTC_DECRYPT)
    {
      if (dropbear_poly1305(cfd, key, in, len, tag) != CRYPT_OK)
        {
          goto out;
        }
  ...
  if (dropbear_chacha(cfd, KEY_HEADER(state), seq, 0, in, out, 4) !=
      CRYPT_OK)
    {
      goto out;
    }

  if (dropbear_chacha(cfd, KEY_MAIN(state), seq, 1, in + 4, out + 4,
                      len - 4) != CRYPT_OK)
    {
      goto out;
    }
  ...
  if (direction == LTC_ENCRYPT)
    {
      if (dropbear_poly1305(cfd, key, out, len, out + len) != CRYPT_OK)
        {
          goto out;
        }
    }
                              ...
                              FAR void *cipher_state)
{
  ...
  ret = dropbear_chacha(cfd, KEY_HEADER(state), seq, 0, in, buf,
                        sizeof(buf));
  close(cfd);
  if (ret != CRYPT_OK)
    {
