...
struct fbvnc_s
{
  ...
  struct fbvnc_cfg_s cfg;        /* Copy, so the caller's may go away */
  pthread_t thread;
  int listensock;
  uint16_t port;
   ...
   * SetEncodings message lands and then used for every rectangle.  Raw
   * is the fallback every RFB client is required to accept.
   */

  int32_t encoding;

  /* When the last update finished, for the rate cap */

  struct timespec lastupdate;

  /* Sending a frame takes as long as the frame is large, and while it
   * is in flight the client's key and pointer messages sit unread in
   * the socket.  The reader and the sender are therefore separate:  the
   * reader only ever parses messages and dispatches input, and hands
   * the sender one token per update the client asks for, so every
  ...
  /* Whether the client is owed a frame.  It is a flag and not a count:
   * a client that asks again while one is being sent is asking for what
   * the screen looks like now, not for two frames, and answering every
   * request separately means sending it a queue of pictures that were
   * already out of date when they left.
   */

  volatile bool updatereq;
  volatile bool clientrun;
  int clientsock;
  struct fbvnc_fmt_s fmt;
...
static int fbvnc_sendall(FAR struct fbvnc_s *dev, int sock,
...
static int fbvnc_recvall(FAR struct fbvnc_s *dev, int sock,
      ...
      if (nrecvd < 0)
        {
          if (errno == EINTR)
            {
              /* A signal is how the daemon is asked to stop.  Anything
               * else was a genuine interruption and the read resumes.
               */
...
static int fbvnc_sendrect(FAR struct fbvnc_s *dev, int sock,
                          ...
                          FAR const struct fbvnc_fmt_s *fmt)
{
  ...
  ret = fbvnc_sendall(dev, sock, &hdr, SIZEOF_RFB_RECTANGE_S(0));
  if (ret < 0)
    {
      return ret;
    }

  row = fb + rect->y * dev->stride + rect->x * VNC_BYTESPP;

  if (fmt->native && rect->x == 0 && rect->w == dev->w)
    {
      ...
      return fbvnc_sendall(dev, sock, row, rect->h * dev->stride);
    }
  ...
  for (y = 0; y < rect->h; y++)
    {
      if (fmt->native)
        {
          ret = fbvnc_sendall(dev, sock, row, rect->w * VNC_BYTESPP);
        }
      else
        {
          fbvnc_cvtrow(row, dev->cvt, rect->w, fmt);
          ret = fbvnc_sendall(dev, sock, dev->cvt,
                                  rect->w * fmt->bytespp);
        }
...
static int fbvnc_tilepalette(FAR struct fbvnc_s *dev, FAR const uint8_t *fb,
                                 ...
                                 FAR uint16_t *bg, FAR uint16_t *fg)
{
  ...
  for (i = 0; i < h; i++)
    {
      row = (FAR const uint16_t *)(fb + (y + i) * dev->stride) + x;
...
static int fbvnc_tilesubrects(FAR struct fbvnc_s *dev, FAR const uint8_t *fb,
                                  uint16_t x, uint16_t y,
                                  uint16_t w, uint16_t h,
...
static int fbvnc_tilepalette_n(FAR struct fbvnc_s *dev,
                               FAR const uint8_t *fb,
                               uint16_t x, uint16_t y,
                               uint16_t w, uint16_t h,
...
static int fbvnc_sendrect_trle(FAR struct fbvnc_s *dev, int sock,
                               FAR const uint8_t *fb,
                               ...
                               FAR const struct fbvnc_fmt_s *fmt)
{
      ...
      for (tx = 0; tx < r->w; tx += VNC_TRLE_TILE)
        {
          ...
          if (used + 1 + tw * th * VNC_BYTESPP > sizeof(buf))
            {
              ret = fbvnc_sendall(dev, sock, buf, used);
              if (ret < 0)
                {
                  return ret;
                }
          ...
          npal    = fbvnc_tilepalette_n(dev, fb, r->x + tx, r->y + ty,
                                            tw, th, palette,
  ...
  if (used > 0)
    {
      return fbvnc_sendall(dev, sock, buf, used);
    }
...
static int fbvnc_sendrect_hextile(FAR struct fbvnc_s *dev, int sock,
                                  FAR const uint8_t *fb,
                                  FAR const struct fbvnc_rect_s *r,
                                  FAR const struct fbvnc_fmt_s *fmt)
{
      ...
      for (tx = 0; tx < r->w; tx += VNC_HEXTILE_TILE)
        {
          ...
          ncolours = fbvnc_tilepalette(dev, fb, r->x + tx, r->y + ty,
                                           tw, th, &bg, &fg);
          ...
          if (ncolours == 2)
            {
              nsub = fbvnc_tilesubrects(dev, fb, r->x + tx, r->y + ty,
                                            tw, th, fg, subrects,
          ...
          *p++ = RFB_HEXTILE_RAW;
          ...
          for (i = 0; i < th; i++)
            {
              memcpy(p, fb + (r->y + ty + i) * dev->stride +
...
static int fbvnc_sendupdate(FAR struct fbvnc_s *dev, int sock,
                                FAR const struct fbvnc_fmt_s *fmt)
{
  ...
  fb = dev->cfg.snapshot(dev->cfg.ctx, rects, VNC_MAX_DIRTY, &nrects);
  ...
  ret = fbvnc_sendall(dev, sock, &hdr, SIZEOF_RFB_FRAMEBUFFERUPDATE_S(0));
  if (ret < 0)
    {
      return ret;
    }
  ...
  for (i = 0; i < nrects; i++)
    {
      ...
      if (dev->encoding == RFB_ENCODING_TRLE && fmt->native)
        {
          ret = fbvnc_sendrect_trle(dev, sock, fb, &rects[i], fmt);
        }
      else
#endif
#ifdef CONFIG_NETUTILS_FBVNC_ENCODING_HEXTILE
      if (dev->encoding == RFB_ENCODING_HEXTILE && fmt->native)
        {
          ret = fbvnc_sendrect_hextile(dev, sock, fb, &rects[i], fmt);
        }
      else
#endif
        {
          ret = fbvnc_sendrect(dev, sock, fb, &rects[i], fmt);
        }
  ...
  syslog(LOG_INFO, "fbvnc: update: %" PRIu32 " rect(s), %" PRIu32
         ...
         nrects, nbytes / VNC_BYTESPP, dev->wirebytes,
...
static int fbvnc_handshake(FAR struct fbvnc_s *dev, int sock,
                           FAR struct fbvnc_fmt_s *fmt)
{
  ...
  ret = fbvnc_sendall(dev, sock, RFB_PROTOCOL_VERSION_3p7, sizeof(version));
  if (ret < 0)
    {
      return ret;
    }

  ret = fbvnc_recvall(dev, sock, version, sizeof(version));
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = fbvnc_sendall(dev, sock, sectypes, sizeof(sectypes));
  if (ret < 0)
    {
      return ret;
    }

  ret = fbvnc_recvall(dev, sock, &selected, sizeof(selected));
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = fbvnc_recvall(dev, sock, &shared, sizeof(shared));
  if (ret < 0)
    {
      return ret;
    }
  ...
  rfb_putbe16(sinit.width, dev->w);
  rfb_putbe16(sinit.height, dev->h);
  ...
  ret = fbvnc_sendall(dev, sock, &sinit, SIZEOF_RFB_SERVERINIT_S(0));
  if (ret < 0)
    {
      return ret;
    }

  ret = fbvnc_sendall(dev, sock, VNC_NAME, namelen);
  if (ret < 0)
    {
      return ret;
    }
...
static void fbvnc_parsepixelfmt(FAR struct fbvnc_s *dev,
...
{
  FAR struct fbvnc_s *dev = arg;
  int sock = dev->clientsock;
  struct timespec now;
  int32_t elapsed;
  int ret;
  ...
  while (dev->clientrun)
    {
       ...
       * out. A client that asks again the instant it has finished
       * parsing, which is what they do while a list is being dragged,
      ...
      if (VNC_MIN_INTERVAL > 0)
        {
          ...
          elapsed = (now.tv_sec - dev->lastupdate.tv_sec) * 1000 +
                    (now.tv_nsec - dev->lastupdate.tv_nsec) / 1000000;
      ...
      ret = fbvnc_sendupdate(dev, sock, &dev->fmt);
      clock_gettime(CLOCK_MONOTONIC, &dev->lastupdate);
...
static void fbvnc_handleclient(FAR struct fbvnc_s *dev, int sock)
{
  pthread_attr_t attr;
  ...
  ret = fbvnc_handshake(dev, sock, &dev->fmt);
  if (ret < 0)
    {
  ...
  dev->encoding = RFB_ENCODING_RAW;
  clock_gettime(CLOCK_MONOTONIC, &dev->lastupdate);

  dev->connected = true;
  if (dev->cfg.on_connect != NULL)
    {
      dev->cfg.on_connect(dev->cfg.ctx);
    }
  ...
  dev->clientsock = sock;
  ret = pthread_create(&dev->sender, &attr, fbvnc_sendthread, dev);
  pthread_attr_destroy(&attr);

  if (ret != 0)
    {
      syslog(LOG_ERR, "fbvnc: cannot start the sender: %d\n", ret);
      sem_destroy(&dev->updatesem);
      return;
    }

  pthread_setname_np(dev->sender, "fbvncsend");

  while (dev->running && dev->clientrun)
    {
      ret = fbvnc_recvall(dev, sock, &msgtype, sizeof(msgtype));
      if (ret < 0)
        {
      ...
      switch (msgtype)
        {
          case RFB_SETPIXELFMT_MSG:
            ...
            ret = fbvnc_recvall(dev, sock, buf, 19);
            if (ret < 0)
              {
                goto teardown;
              }

            fbvnc_parsepixelfmt(dev, buf, &dev->fmt);
            break;

          case RFB_SETENCODINGS_MSG:
            ret = fbvnc_recvall(dev, sock, buf, 3);
            if (ret < 0)
              {
                goto teardown;
              }
            ...
            for (i = 0; i < nencodings; i++)
              {
                ret = fbvnc_recvall(dev, sock, buf, 4);
                if (ret < 0)
                  {
                    goto teardown;
                  }
            ...
            if (hashextile)
              {
                dev->encoding = RFB_ENCODING_HEXTILE;
              }
#endif
            ...
            if (hastrle)
              {
                dev->encoding = RFB_ENCODING_TRLE;
              }
#endif

            syslog(LOG_INFO, "fbvnc: client offered %u encodings, "
                             "using %s\n", nencodings,
                   dev->encoding == RFB_ENCODING_TRLE ? "TRLE" :
                   dev->encoding == RFB_ENCODING_HEXTILE ?
                   "Hextile" : "Raw");
            break;

          case RFB_FBUPDATEREQ_MSG:
            ret = fbvnc_recvall(dev, sock, buf, 9);
            if (ret < 0)
              {
                goto teardown;
              }
            ...
            if (buf[0] == 0 && dev->cfg.on_invalidate != NULL)
              {
                dev->cfg.on_invalidate(dev->cfg.ctx);
              }
          ...
          case RFB_KEYEVENT_MSG:
            ret = fbvnc_recvall(dev, sock, buf, 7);
            if (ret < 0)
              {
                goto teardown;
              }
            ...
            if (dev->cfg.on_key != NULL)
              {
                dev->cfg.on_key(dev->cfg.ctx, rfb_getbe32(&buf[3]),
                                buf[0] != 0);
              }
            break;

          case RFB_POINTEREVENT_MSG:
            ret = fbvnc_recvall(dev, sock, buf, 5);
            if (ret < 0)
              {
                goto teardown;
              }
            ...
            if (dev->cfg.on_pointer != NULL)
              {
                dev->cfg.on_pointer(dev->cfg.ctx, rfb_getbe16(&buf[1]),
          ...
          case RFB_CLIENTCUTTEXT_MSG:
            ...
            for (i = rfb_getbe32(&buf[3]); i > 0; i--)
              {
                ret = fbvnc_recvall(dev, sock, buf, 1);
                if (ret < 0)
                  {
                    goto teardown;
                  }
...
static void fbvnc_serve(FAR struct fbvnc_s *dev)
{
  struct timeval tv;
  int listensock = dev->listensock;
  int clientsock;
  ...
  while (dev->running)
    {
      ...
      fbvnc_handleclient(dev, clientsock);

      close(clientsock);
      dev->connected = false;

      if (dev->cfg.on_disconnect != NULL)
        {
          dev->cfg.on_disconnect(dev->cfg.ctx);
        }
    }
...
FAR struct fbvnc_s *fbvnc_start(FAR const struct fbvnc_cfg_s *cfg)
{
  ...
  dev = calloc(1, sizeof(struct fbvnc_s));
  if (dev == NULL)
    {
      set_errno(ENOMEM);
      return NULL;
  ...
  dev->cfg    = *cfg;
  dev->w      = cfg->width  != 0 ? cfg->width  : VNC_WIDTH;
  dev->h      = cfg->height != 0 ? cfg->height : VNC_HEIGHT;
  dev->stride = cfg->stride != 0 ? cfg->stride : dev->w * VNC_BYTESPP;
  dev->port   = cfg->port   != 0 ? cfg->port   : VNC_PORT;
  dev->encoding = RFB_ENCODING_RAW;
  ...
  dev->listensock = socket(AF_INET, SOCK_STREAM, 0);
  if (dev->listensock < 0)
    {
      free(dev);
      return NULL;
  ...
  optval = 1;
  setsockopt(dev->listensock, SOL_SOCKET, SO_REUSEADDR, &optval,
             sizeof(optval));
  ...
  if (bind(dev->listensock, (FAR struct sockaddr *)&addr,
           sizeof(addr)) < 0 ||
      listen(dev->listensock, 1) < 0)
    {
      int errcode = errno;

      close(dev->listensock);
      free(dev);
      set_errno(errcode);
      return NULL;
    }
...
int fbvnc_run(FAR struct fbvnc_s *dev)
{
  ...
  /* The caller's thread is the server.  Serving here rather than from a
   * thread of our own is what lets a daemon whose only other job is to
   * wait for this avoid paying for a second stack.
   */

  fbvnc_serve(dev);

  /* Nothing refers to the instance once the loop is out */
...
void fbvnc_stop(FAR struct fbvnc_s *dev)
{
  ...
  /* Asking the loops to leave is enough:  the poll() timeout in the
   * accept path bounds how long it takes to notice, and it closes the
   * listening socket on its way out.
   */
