...
#define SYSTEM_INIT_SERVICE_GENTLE_KILL_TIMEOUT 200
                            ...
                            node)
    {
      if (check_flags(service, SVC_RESTARTING))
               ...
               check_flags(service, SVC_DISABLED))
        {
          ...
          ms = TIMESPEC2MS(diff);
          if (ms >= SYSTEM_INIT_SERVICE_GENTLE_KILL_TIMEOUT)
            {
          ...
          min = MIN(min, SYSTEM_INIT_SERVICE_GENTLE_KILL_TIMEOUT - ms);
        }
...
{
  FAR char *argv[CONFIG_SYSTEM_INIT_SERVICE_ARGS_MAX];
  ...
  if (create)
    {
      ...
      s->restart_period = CONFIG_SYSTEM_INIT_SERVICE_RESTART_PERIOD;
