 ...
 *
 * Description:
 *   Find the position of the newline character separating the syslog entry.
 *
 * Return: Index of the newline character, -1 if not found.
 ****************************************************************************/

static ssize_t find_endpos(char *buf, size_t n)
{
  ssize_t pos;
  ...
  for (; ; )
    {
      ...
      endpos = find_endpos(buffer, bufpos + bread);
      ...
      if (endpos < 0)
        {
           ...
           * need to:
           *
           * 1) Read more and try again if there's still room in our buffer
           *
           * 2) Acknowledge that the syslog entry is too long for our buffer
           * size, and skip it since we can't construct a UDP packet if
           * that's the case.
           */

          if (bufpos + bread < sizeof(buffer))
            {
