...
#define UNITY_IGNORE_AND_BAIL { Unity.CurrentTestIgnored = 1; UnityAbort(); }
...
static void UnityAbort(void) noreturn_function;
...
void UnityDefaultTestRun(UnityTestFunction Func, const char* FuncName, const int FuncLineNum)
{
    ...
    Unity.NumberOfTests++;
    if (TEST_PROTECTED(setUp) == 0)
    {
        TEST_PROTECTED(Func);
    }
    if (!(Unity.CurrentTestIgnored))
    {
        TEST_PROTECTED(tearDown);
    }
    UnityConcludeTest();
}
...
static void* UnityProtectedThread(pthread_addr_t value)
{
    UnityTestFunction function = (UnityTestFunction)value;
    function();
    return NULL;
}
...
int UnityProtect(UnityTestFunction function, _U_UINT timeout_msec)
{
    ...
    if (result == OK)
    {
        ...
        result = pthread_create(&timeout_thread, NULL, UnityTimeoutThread, (pthread_addr_t)&action);
        if (result == OK)
        {
